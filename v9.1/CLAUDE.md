...
52 mm of it at `perchSize`. The lift is now `perchLegDrop × size`, so the lowest leg tip
lands on the surface and the body is held 72 mm clear of it.
...
top, then three pairs of legs descending, the longest reaching −0.1406. Cross-check:
...
legs sink as the butterfly grew. The body plane stays vertical in the rest pose whatever the
palm's tilt or the broadside turn do (it contains the body axis and the palm normal), so the
legs always hang straight down that normal and this one number is the whole correction.
