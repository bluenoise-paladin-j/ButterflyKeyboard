...
AFRAME.registerComponent('butterfly-collection', {
  init: function () {
    ...
    ['maxCollected', 'colRadMin', 'colRadMax', 'colHgtMin', 'colHgtMax',
     ...
     'perchSize', 'perchLift', 'perchLegDrop', 'perchFollow', 'perchDwell', 'perchSettle', 'perchRest',
  ...
  //  it -- body, then three pairs of legs down to CFG.perchLegDrop per
  //  unit model size. Placing the ORIGIN on the palm (which is what the
  //  first pass did) therefore buried the whole body and every leg in the
  //  hand, about 52 mm of it at perchSize.
  //
  //  Along the palm's own normal, so it stands on a tilted hand rather
  //  than hovering level above it -- and the body plane stays vertical in
  //  the rest pose whatever the tilt or the broadside turn do, so the legs
  //  always hang straight down that normal.
  //
