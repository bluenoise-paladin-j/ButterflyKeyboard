...
var CFG = {
  ...
  //  hinge, and the body sprite hangs below it: body, then three pairs of
  //  legs reaching down to local y -0.1406 at scale 1. So the butterfly was
  //  planted wings-deep in the hand with every leg buried, about 52 mm of
  //  it at perchSize.
  //
  //  perchLegDrop is that number, measured off the body alpha itself rather
  //  than guessed: decode BODY_ALPHA, threshold at the material's own
  //  alphaTest of 0.5, and the lowest painted pixel lands at local y
  //  -0.1406. (Cross-check: CFG.tagBodyDrop, measured independently across
  //  flap and roll for the name tag, is -0.135.) Quoted per unit model
  //  size, same convention as tagBodyDrop, so it scales with the butterfly.
  //
  //  The body plane stays vertical in the rest pose whatever the palm's
  //  tilt or perchYaw does -- it contains the body axis and the palm
  //  normal -- so the legs always hang straight down the normal and this
  //  one number is the whole correction.
  perchLegDrop: 0.1406,     // per unit model size: how far the lowest leg tip hangs below the model origin
  perchLift:   0.002,       // metres of clearance between that leg tip and the palm. Negative sinks it in
