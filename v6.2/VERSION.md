...
A janitorial pass — no interaction, composition, capture flow or typography
change that you would see on a clean load. Three latent bugs fixed, dead code
removed, the generation seam left exactly as it was.

**Three latent bugs, all from `CFG` keys that went missing between v4 and v6 while
the code using them stayed.** Each produced `NaN` and rendered nothing, so none
was visible on the Quest — which is why they survived v6's on-headset sign-off.

- **`CFG.arcSpan` under `2π` collapsed the whole swarm.** `pathAt()`'s partial-arc
  branch reads `k.centre` / `k.swing`, and nothing had set them since v4's
  `buildKeys` rewrite dropped the two lines. Setting `arcSpan = 3.4` — which this
  file and `CLAUDE.md` both recommend trying — put every butterfly at `NaN`. The
  two lines are restored in `buildKeys` (from v3), computed from `CFG.arcSpan` at
  build time. **Zero effect at the default full circle**; the branch stays unread.
- **The blown-up letter behind a highlighted butterfly never rendered.**
  `tickBig()` multiplied by undefined `CFG.bigSize` / `bigBehind` / `bigOpacity`.
  Removed outright (`tickBig`, `this.big`, `UI.bigLetter`, `bigTex`) rather than
  revived — it had been absent through the whole v6 exhibition run.
- **The satellites (a hollow letter on 7 of the 26 butterflies) never rendered.**
  `dress()` sized them against undefined `CFG.satMin` / `satMax` / `satAngular`
  and `UI.hollow` was passed an undefined colour. Removed (`st.satellite` /
  `st.satChar`, the `k.num` object, `UI.hollow`). Removing `st.satellite` /
  `st.satChar` does **not** shift `style.js`'s roll sequence — neither drew from
  the PRNG — so every letter's composition is byte-identical to v6.1.

**Dead code removed:**

- `ui.js` — `UI.wordmark` / `UI.rule` / their caches (scatter leftovers; the
  scatter was deleted in v6), the unused `OVERPRINT` constant.
- `hands.js` — `rig.ray()` (interact.js casts its own shoulder ray since v6.1),
  `rig.openness` and its three finger-tip joint constants (never read), the
  scratch vectors they used.
- `bfly-model.js` — five unused fields off `build()`'s return (`leftPivot`,
  `rightPivot`, `wingMat`, `bodyMat`, `letterMat`) and the `BASE` export.
  `model` / `setColor` / `setOpacity` / `flap` / `dispose` stay; `setColor` is
  kept on purpose for the generation stage.
- `config.js` — a duplicated `panelR` line, the unused `bg`.

**Kept deliberately** — `hand-rig.debug()` (the documented on-headset
hand-tracking bring-up readout; hand tracking still wants a physical pass), and
the entire generation seam: `app.js`, `wing-gen.js` / `rolltable.js` /
`textures.js` / `wing-tex.js` and the full `Wings.*` API, even the parts nothing
calls yet.

## What changed from v6 (the v6.1 pass, unchanged in v6.2)
...
| nothing decorative is pickable | 26 targets, all butterflies — the thrown type, ghosts and lattices never enter the list |
...
| v6.2 cleanup, no visible change | 26 keys, all positions finite; highlight still scales to `hiScale` and inverts the letter; CAT typed and accepted end to end on a clean load; `arcSpan = 3.4` now keeps every butterfly finite and in front |
...
**Scene weight: ~260 objects** — 144 meshes, ~102 sprites, 14 lines, and `alphaTest`
materials do not batch, so that is roughly the draw call count. (v6.1 was ~268; v6.2's
cleanup dropped the 7 satellite sprites and the 1 blown-up-letter sprite.) v2 sized its
budget at 180 for a room of sixty butterflies, so this is still over it. If it will not
hold 72 Hz, the levers in order of how much they cost the design:
...
Neither is load-bearing for the interaction.
...
makes hunting more interesting to look at and no easier to do. Dropping it to ~3.4 in
`config.js` (before load) to sweep an arc in front of the visitor instead is now actually
possible — v6.2 fixed the `NaN` that made any value under `2π` collapse the swarm.
