# v10.2 — the staff reset, and a shorter fuse, 9 September 2026

Two changes, both about the exhibition floor rather than the visitor. (There is no v10.1;
the amended recall recording went into v10 in place.)

```bash
sh tools/reach/run.sh              # 221 assertions, five suites
```

## The timeout was too long

Forty-five seconds of nothing before the room handed over. `guideQuietMs` 30 s → **15 s**,
`guideQuietGrace` 15 s → **10 s**: **25 s** from the last thing a visitor did, and the clock
is still frozen through the reveal, through a butterfly coming over or sitting on a hand, and
through a cue that is still teaching them something.

**What this buys back is a risk window, and it is worth naming.** Right after the recall cue
the visitor is *searching* the kaleidoscope for their butterfly, and pure looking is not
activity — pointing at them to find theirs is. If sessions start ending under people on the
floor, `guideQuietMs` is the line. It is only affordable because the fist below now covers
what the long timer was really insuring against.

## Two closed fists, held up, for five seconds

The exhibition team's hard reset. It ends the session **at once and silently**, exactly as
taking the headset off does — the keyboard resets, anything with the visitor flies home, the
start flower comes back, and **the collection is not touched**. Nothing is announced: the
visitor never needs to know it exists, and the room returning to the flower is the whole
confirmation.

### The collision was with the PINCH, not the palm

A fist is trivially not a flat palm — `palmFlat` wants the fingertips ≥ `palmFlatExtend`
(1.45) palm widths from the wrist and a fist is the far end of that same measurement, so the
two are mutually exclusive by construction and nothing had to be done about it.

**A pinch with the other three fingers curled is a different matter**, and that is how a
great many people pinch. Measured on the joint model:
...
**The first draft used the mean and a curled pinch read as a fist.** On the mean the two sit
0.18 apart and the threshold has to thread that band — too tight for real hands. On the
**farthest** tip the gap is **0.32**, because a fist has every tip in against the palm while a
curled pinch holds two of them out *in front* of it, and that is the actual difference
between the gestures. `fistBall` is 0.80, with 0.14 of margin below and 0.19 above.

Measuring against the **palm centre** over **all five** tips is the other half of it:
`palmFlat`'s own measure is fingertip-to-*wrist* over *four* fingers, which is right for "are
the fingers extended" and useless here — nothing that ignores the thumb can separate these
two poses.

### Three other things it needs

- **Held UP.** Hands hang closed by a person's side all the time, so `fistPose` is
  `balled && palmRaised` — the same headset-relative raise test the offer uses.
- **BOTH hands** (`fistHands` 2). This is the safety rather than a formality: a pinch is one
  hand doing the work while the other hangs, so two balled hands held up together is a pose
  no visitor reaches by accident and no interaction in the piece produces.
- **A blink does not restart the five seconds.** `holdFist` differs from `holdOffer` in
  ...
  (`fistGraceMs` 400) rather than clearing the arming clock. Quest hand tracking drops a
  frame or two at a time, and a five-second hold that reset on every blink would be unusable.

### …and a fisted hand no longer activates anything

One line in `interact.js`. In a closed fist the thumb tip sits near the curled index, close
enough to the pinch threshold to trip it — and this gesture asks for two of them held up for
five seconds, so without it a reset would fling letters into the name on its way. Worth
having on its own account too: a fist reading as a pinch is a familiar hand-tracking
annoyance.

### Latched

One reset per gesture. The hands go on being held while the person works out whether it
worked, so an unlatched check would reset the room every frame and the flower would never
come back. Both hands have to open before it can fire again.

## Tested

`sh tools/reach/run.sh` — **221 assertions**, five suites. `test-fist.js` is new and prints
the four poses before asserting on them, because those numbers *are* the argument for the
threshold. It covers the pinch collision from both sides, a fist never being an offer and an
offer never being balled, the raise gate, the five-second hold, a blink that does not cost
the hold and a genuine release that does, and the two-hand rule. `test-guide.js` gains the
latch: exactly one reset, ten more seconds of holding changing nothing, one fist not being
the gesture, and re-arming once the hands open.

Driven in the browser too, with the synthetic hand written into the real rigs' joint buffers
so the real detector runs on a real clock: `ballT` **0.661** for a fist against **1.428** open,
`fistPose` true on both hands, `palmFlat` false on both — and the session ending at **5.2 s**,
twice over, with the flower back at alpha 1.
...
> arithmetic goes to nonsense — it looked like a broken latch and was not. In the scene there
> is only ever one clock, because `holdFist` is only ever called from `tick`.

## Files

| | |
|---|---|
| `js/hands.js` | `ALL_TIPS`, the ball measurement, `fistPose`, `holdFist`, `Hands.fists()` / `resetAsked()` |
| `js/guide.js` | the latched check, before the state switch and outside the mode gate |
| `js/interact.js` | **one line**: a fisted hand does not activate |
| `js/config.js` | four `fist*` knobs, and the two shortened timeouts |
| `tools/reach/` | `test-fist.js`, the latch block in `test-guide.js`, `run.sh` |
| `tools/knobs.html` | regenerated — **281 tunables**, 6 flagged |

Everything else is byte-identical to v10.

## What is not here

- **No feedback while the five seconds run.** The room changing is the only confirmation, so
  a staff member holding the pose has nothing to look at until it fires. A visible arming
  ramp is the obvious next thing if the floor asks for one.
- **`fistHands = 1`** makes it a one-handed gesture. The two-hand rule is what makes it
  un-triggerable by accident; do not drop it without re-reading the pinch table above.
- Still **not run on a physical Quest.** Every hand path here is verified against the
  synthetic joint model, which is the standard v9's palm work was held to.
