...
ok(saidSince(mark).join(',') === CFG.voIntro.join(','),
   'and the whole opening was asked for, got ' + saidSince(mark).join(','));
...
print('\n== v10.2: two fists are the exhibition team\'s hard reset ==');
//  LATCHED, and that is the part worth asserting: the gesture is held for
//  five seconds and then goes on being held while the person works out
//  whether it worked, so an unlatched check would reset the room every
//  frame and the flower would never come back.
var RESETS = 0;
window.addEventListener('guide:reset', function () { RESETS++; });
Guide.handover(); frames(4); Guide.begin(); until(inState('live'), 30);
KB.resets = 0; COL.busy = true;
RIGS.left.fist = true; RIGS.right.fist = true;
frames(1);
ok(Guide.state() === 'idle', 'two fists end the session at once, state ' + Guide.state());
ok(RESETS === 1, 'exactly one reset, got ' + RESETS);
ok(KB.resets === 1 && COL.busy === false, 'and the room is put back: keyboard reset, butterflies sent home');
ok(saidSince(PLAYED.length)[0] === undefined, 'silently -- no farewell to an empty headset');
frames(200);                                // ten more seconds of holding them
ok(RESETS === 1, 'holding them does NOT keep resetting: still ' + RESETS);
ok(Guide.flowerAlpha() > 0.9, 'and the flower comes back while they are still held, alpha ' + Guide.flowerAlpha().toFixed(2));

print('\n   -- one fist is not the gesture --');
Guide.begin(); until(inState('live'), 30);
RIGS.left.fist = false;                     // one hand opens
frames(4);
ok(Guide.state() === 'live', 'a session survives one fist, state ' + Guide.state());
ok(RESETS === 1, 'and nothing fired, still ' + RESETS);

print('\n   -- and it re-arms once the hands open --');
RIGS.right.fist = false;
frames(4);
RIGS.left.fist = true; RIGS.right.fist = true;
frames(2);
ok(RESETS === 2, 'a second gesture fires a second reset, got ' + RESETS);
RIGS.left.fist = false; RIGS.right.fist = false;
frames(4);
