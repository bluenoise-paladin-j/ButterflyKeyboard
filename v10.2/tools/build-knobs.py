...
#  constant that predates this version, 'new' for one v10.2 introduced,
...
FILES = [
 dict(f='js/config.js', hue=214, blurb='Every number the piece is built from. 217 keys. Almost everything you will want to move is here.', groups=[
  ('the room', [
  ...
  ('the posture that means “land here” (hands.js:palmPose)', [
   ('fistBall','0.80',871,"The FARTHEST fingertip from the palm centre, in palm widths — not the mean of them. A fist has every tip in against the palm while a curled pinch holds two of them out in front of it, which is the actual difference between the gestures. Fist 0.66, curled pinch 0.99, open hand 1.43.",'new'),
   ('fistHoldMs','5000',872,"How long two closed fists have to be held up before the session ends. The exhibition team's hard reset; the visitor never needs to know it exists.",'new'),
   ('fistGraceMs','400',873,"Milliseconds of lost tracking that neither restart the five-second hold nor drop a held one. The one way holdFist differs from holdOffer, and over a hold this long it is what makes it usable.",'new'),
   ('fistHands','2',874,"How many fists it takes. TWO is the safety rather than a formality: a pinch is one hand doing the work while the other hangs, so two balled hands held up together is a pose no visitor reaches by accident.",'new'),
  ...
  ('the session — v10’s guide, and the voice that runs it', [
   ('guideQuietMs','15000',930,"Milliseconds of no interaction before the nudge. SHORT on purpose: an exhibition floor hands the headset over fast, and ending is non-destructive — the collection is never touched, so a visitor ended early just presses start again. v10.2: was 30 s — 45 seconds of nothing before the room handed over was too long for an exhibition floor, and the two-fist reset now covers what the long timer was insuring against.",'changed'),
   ('guideQuietGrace','10000',931,"...and this much more before the farewell. ANY activity cancels and puts the session back, so nobody is cut off mid-action. v10.2: was 15 s.",'changed'),
   ...
   ('voCues','{ welcome, spell, pinch, recall }',929,"Cue name → file, relative to index.html. A cue with no entry is a SILENT NO-OP that still reports itself finished, so the guide runs with whatever subset of the tracks exists — nudge and farewell are not recorded. See audio/README.md.",''),
   ('voIntro','[welcome, spell, pinch]',960,"The opening, in order — three recordings played back to back as ONE cue. playing() stays true across the gaps, so the silence between two of them is never mistaken for the narration having finished.",''),
   ('voGap','0.45',961,"Seconds of silence between two cues in a sequence.",''),
  ]),
  ('palette', [
...
w('<div class="wrap">\n')
w('<header class="top">\n')
w('<p class="eyebrow">Butterflies VR · v10.2 · versions/v10.2</p>\n')
...
w('<span><b>%d</b> new or changed in v10.2</span>' % newn)
w('</div>\n</header>\n')
...
for f in FILES:
    ...
    for gname, rows in f['groups']:
        ...
        for name, val, line, note, flag in rows:
            ...
            elif flag=='changed': fl = '<span class="flag chg">v10.2</span>'
...
w('<span><code>versions/v10.2/</code></span>')
