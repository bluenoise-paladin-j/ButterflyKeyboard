...
| | |
|---|---|
| `guideQuietMs` **10 s** | of no interaction → the **nudge**. *v10.2: was 30 s* |
| `guideQuietGrace` **5 s** | more → the **farewell**. Any activity cancels and puts the session back. *v10.2: was 15 s* |
...
making ten seconds safe rather than a hair trigger: the reveal is ~14 s of watching by design,
