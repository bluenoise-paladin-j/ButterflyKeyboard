...
//      rig.fist           v10.2 -- a closed fist, held up, held for
//                         CFG.fistHoldMs. The exhibition team's hard
//                         reset; see palmPose's fist block
...
//  ALL FIVE tips, for the fist test (v10.2). The thumb is in here and
//  deliberately NOT in PALM_FINGERS below: palmFlat is about the fingers
//  being extended, and a thumb is extended in a pinch too.
var ALL_TIPS = [HAND_JOINT.thumbTip, HAND_JOINT.indexTip, HAND_JOINT.middleTip,
...
AFRAME.registerComponent('hand-rig', {
  ...
  init: function () {
    ...
    //  v10.2 -- THE STAFF RESET. A balled hand, held up, held still. See
    //  palmPose's fist block and holdFist below.
    this.balled = false;                      // all five tips in to the palm
    this.fistPose = false;                    // ...and raised
    this.fist = false;                        // ...and held for fistHoldMs
    this._fistSince = null; this._fistBad = null;
    this._okSince = null;
    this._badSince = null;
  ...
  palmPose: function (p) {
    ...
    //  v10.2 -- A CLOSED FIST, as the exhibition team's hard reset.
    //
    //  Measured off the PALM CENTRE and over ALL FIVE tips, not off the
    //  wrist and not over four. Both of those choices are the whole
    //  reason this does not collide with a pinch:
    //
    //    - `palmFlat` measures fingertip-to-WRIST and skips the thumb,
    //      which is right for "are the fingers extended" and useless
    //      here: a pinching hand with the other three curled scores
    //      almost exactly like a fist on that measure.
    //    - the difference between the two is WHERE THE INDEX AND THUMB
    //      TIPS ARE. In a fist they are in against the palm with
    //      everything else; in a pinch they are held out in front of it,
    //      touching each other. Distance-to-palm-centre over all five
    //      separates those; nothing that ignores the thumb can.
    //
    //  ...and it is the FARTHEST tip that decides, not the mean of them.
    //  Measured on the joint model (tools/reach/test-fist.js prints all
    //  four poses):
    //
    //                            mean     farthest
    //      fist                  0.533      0.662
    //      pinch, fingers curled 0.712      0.985   <- the collision
    //      pinch, fingers out    1.169      1.169
    //      open hand             1.212      1.428
    //
    //  On the MEAN the curled pinch sits 0.18 from a fist and the
    //  threshold has to thread a 0.18-wide band -- too tight for real
    //  hands. On the FARTHEST tip the gap is 0.32, because a fist has
    //  every tip in against the palm while a curled pinch holds two of
    //  them out in front of it, and that is the actual difference between
    //  the two gestures. The first draft used the mean and a curled pinch
    //  read as a fist.
    //
    //  The cost of taking a max is sensitivity to one badly-tracked
    //  joint -- which is what holdFist's grace is for: a noisy frame
    //  costs a frame, not the hold.
    var ball = 0;
    for (var t = 0; t < ALL_TIPS.length; t++) {
      var tp = this.jointPos(p, ALL_TIPS[t], this._pa);
      var d = tp.distanceTo(this.palm);
      if (d > ball) { ball = d; }
    }
    this.ballT = ball / this.palmWidth;
    this.balled = this.ballT <= CFG.fistBall;
    //  RAISED as well, on the same test the offer uses. Hands hang closed
    //  by a person's side all the time; a fist held UP is a decision.
    this.fistPose = this.balled && this.palmRaised;
  },

  //  THE FIST, held. Same shape as holdOffer with one difference that
  //  matters over five seconds: a bad frame does not restart the arming
  //  clock, it only starts a grace. Quest hand tracking drops a frame or
  //  two at a time, and a five-second hold that reset on every blink
  //  would be unusable in a room with any hand occlusion in it.
  holdFist: function (ok, now) {
    if (ok) {
      this._fistBad = null;
      if (this._fistSince === null) { this._fistSince = now; }
      if (!this.fist && (now - this._fistSince) >= CFG.fistHoldMs) { this.fist = true; }
      return;
    }
    if (this._fistBad === null) { this._fistBad = now; }
    if ((now - this._fistBad) >= CFG.fistGraceMs) {
  ...
  tick: function (time) {
    ...
    this.holdFist(this.fistPose, time);
  },
  ...
  debug: function () {
    ...
    return {
      ...
      ball: this.ballT, balled: this.balled, fist: this.fist,
...
var Hands = (function () {
  ...
  //  v10.2 -- IS THE EXHIBITION TEAM ASKING FOR A RESET? CFG.fistHands
  //  closed fists, each held up and held for fistHoldMs.
  //
  //  TWO HANDS BY DEFAULT, and that is the safety rather than a
  //  formality: a pinch is one hand doing the work while the other
  //  hangs, so two balled hands held up together is a pose no visitor
  //  reaches by accident and no interaction in the piece produces. It
  //  also cannot be pinching anything -- see interact.js, where a fisted
  //  hand does not activate.
  function fists() {
    var n = 0, sides = ['left', 'right'];
  ...
  function resetAsked() { return fists() >= CFG.fistHands; }
  ...
  return {
    fists: fists, resetAsked: resetAsked,
