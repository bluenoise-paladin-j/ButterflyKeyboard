  ...
  tick: function (time, delta) {
    ...
    for (var i = 0; i < this.pointers.length; i++) {
      ...
      if (p.kind === 'hand') {
        ...
        p.closed = closed;
        //  v10.2 -- A BALLED HAND IS NOT POINTING AT ANYTHING. In a
        //  closed fist the thumb tip sits near the curled index, which is
        //  close enough to the pinch threshold to trip it -- and the
        //  exhibition team's reset asks for two of them held up for five
        //  seconds, so without this a reset would fling letters into the
        //  name on its way. It is worth having on its own account too: a
        //  fist reading as a pinch is a familiar hand-tracking annoyance.
        if (rig.fist) { fire = false; }
      } else {
        fire = !!p.click;
        p.click = false;
      }
