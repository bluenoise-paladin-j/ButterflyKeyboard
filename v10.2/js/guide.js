...
var Guide = (function () {
  ...
  ['guideUnlockAt', 'guideWelcomeMax', 'guideQuietMs', 'guideQuietGrace',
   ...
   'fistBall', 'fistHoldMs', 'fistGraceMs', 'fistHands',
  ...
  var booted = false;
  var fistLatch = false;       // v10.2 -- one reset per gesture, not one per frame
  ...
  function begin() {
    ...
    Voice.play(CFG.voIntro);         // three recordings, played as one
    emit('guide:started');
  }
  ...
  function tick(dt) {
    ...
    //  v10.2 -- THE EXHIBITION TEAM'S HARD RESET. Two closed fists held
    //  up for five seconds ends the session at once and silently, exactly
    //  as taking the headset off does. LATCHED, so it fires once per
    //  gesture and both hands have to open again before it can fire
    //  another -- otherwise holding the pose would reset the room every
    //  frame and the flower would never come back.
    //
    //  Checked before the state switch and outside the mode gate below,
    //  so it works from any state including a stuck one. Nothing about it
    //  is announced: the visitor never needs to know, and the room
    //  visibly returning to the flower is the confirmation.
    if (typeof Hands !== 'undefined' && Hands.resetAsked && Hands.resetAsked()) {
      if (!fistLatch) { fistLatch = true; toIdle(true); emit('guide:reset'); }
    } else {
      fistLatch = false;
    }
    ...
    if (state === WELCOME) {
      ...
      //  `live` once the welcome has finished talking.
      if (stateT >= CFG.guideWelcomeMax ||
          (stateT >= CFG.guideUnlockAt && !Voice.playing())) { toState(LIVE); }
      runQuiet(dt);
      return;
    }
