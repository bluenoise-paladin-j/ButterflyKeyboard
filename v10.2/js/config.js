...
var CFG = {
  ...
  //  v10.2: 30000/15000 -> 10000/5000. FIFTEEN SECONDS from the last
  //  thing a visitor did to the room handing over, where v10 took
  //  forty-five. An exhibition floor turns people over fast and ending is
  //  non-destructive, so the cost of ending early is that they press the
  //  flower again.
  //
  //  This is also the ONLY automatic way a session ends, and it carries
  //  more weight than it did in v10: a staff hard-reset gesture was built
  //  and removed (see the note below), so the two ends are this and the
  //  headset coming off.
  //
  //  The risk window it buys back, and the line to move if the floor
  //  reports sessions ending under people: right after the recall cue the
  //  visitor is SEARCHING the kaleidoscope for their butterfly, and pure
  //  looking is not activity -- pointing at them to find theirs is. The
  //  clock is still frozen through the reveal, through a butterfly coming
  //  over or sitting on a hand, and through a cue that is still teaching
  //  them something, so these ten seconds are ten seconds of a visitor
  //  who could be acting and is not.
  guideQuietMs:     10000,  // ms of no interaction -> the nudge
  guideQuietGrace:   5000,  // ...then this much more -> the farewell. Any activity cancels
