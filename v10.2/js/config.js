...
var CFG = {
  ...
  //  The reveal solves the same problem the same way -- presentRise.
  ...
  //  ---- v10.2: the exhibition team's hard reset ----
  //  TWO CLOSED FISTS, HELD UP, FOR FIVE SECONDS ends the session at
  //  once and silently, exactly as taking the headset off does. The
  //  visitor never needs to know it exists; it is for the people tending
  //  the piece, for a stuck session or a visitor who walks off wearing
  //  it. Nothing is announced -- the room returning to the start flower
  //  is the confirmation.
  //
  //  IT DOES NOT COLLIDE WITH THE PALM, by construction: palmFlat wants
  //  the fingers extended (>= palmFlatExtend of a palm width from the
  //  wrist) and a fist is the far end of that same measurement.
  //
  //  IT COULD HAVE COLLIDED WITH THE PINCH, and that is what fistBall is
  //  shaped against -- see hands.js:palmPose and tools/reach/test-fist.js.
  //  A pinch with the other three fingers curled is nearly a fist on any
  //  measure that ignores the thumb; measuring ALL FIVE tips against the
  //  PALM CENTRE separates them, because a pinch holds the index and
  //  thumb out in front of the palm and a fist does not.
  //  The FARTHEST fingertip from the palm centre, in palm widths, not the
  //  mean of them -- a fist has every tip in against the palm while a
  //  curled pinch holds two of them out in front of it, which is the
  //  actual difference between the gestures. Fist 0.66, curled pinch
  //  0.99, open hand 1.43; 0.80 sits in the gap that matters.
  fistBall:      0.80,      // farthest tip-to-palm distance, in palm widths
  fistHoldMs:    5000,      // held this long before it fires
  fistGraceMs:    400,      // ms of lost tracking that neither restarts the hold nor drops it
  fistHands:        2,      // how many fists it takes. 2 is what makes it un-triggerable by accident
  ...
  //  v10.2: 30000/15000 -> 15000/10000. Forty-five seconds of nothing
  //  before the room handed over was too long for an exhibition floor,
  //  and the fist reset below now covers the case the long timer was
  //  really insuring against (a visitor who wanders off still wearing
  //  it). The one thing this buys back is the risk window: right after
  //  the recall cue a visitor is SEARCHING the kaleidoscope for their
  //  butterfly, and pure looking is not activity. Pointing at them to
  //  find theirs is, which is what makes 15 s liveable -- but if the
  //  floor reports sessions ending under people, this is the line.
  guideQuietMs:     15000,  // ms of no interaction -> the nudge
  guideQuietGrace:  10000,  // ...then this much more -> the farewell. Any activity cancels
