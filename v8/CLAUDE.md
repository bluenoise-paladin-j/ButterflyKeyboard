...
The generation stage (below) always ran a `present` → `joining` → `orbit` beat on a
freshly grown butterfly. v7.2 art-directs the first two states; `orbit` is untouched, and
replayed butterflies still skip straight to it.

**`present` — flat, wings to the visitor, breathing.** `tickPresent` rises the butterfly
into the spot ~0.8 m ahead (following the head on a lag, as before). What changed:

- **The flight's `presentRoll` cannot do this.** It only ever presents a *three-quarter*
  aspect — the butterfly there is yawed to face the visitor, so the body axis points at
  them and the wing plane (which contains that axis) can never face them. Verified in the
  running scene: sweeping the model roll through a full turn, the wing-to-camera aspect
  never exceeds ~0.18. So `present` bypasses `presentRoll` entirely: the **group carries
  no yaw**, and `_flatQuat()` builds a look-at quaternion for the model directly — local
  +Y (the wing normal) → the camera, local +X (the head) → **down** (head-up renders the
  butterfly upside down — the wing slice has the forewing above the seam and the plane's
  UVs put that at local −X) — slerped in as it rises. Measured aspect while held:
  **≈ 0.99**. `presentRoll` stays *verbatim* from `keyboard.js`.
- The wings do a **slow, shallow breath** (`CFG.revealFlapAmp` 0.11 rad at
  `CFG.revealFlapRate` 1.9 rad/s ≈ 0.3 Hz) instead of the full ~3–4 Hz flight beat.
  `bm.flap`'s two pivots mirror, so the wing tips rise and fall *together* — it reads as
  the wings easing open and shut a little, and the aspect barely wavers (0.99–1.0,
  ±0.006 measured).
- `presentArrive` 1.0 s, `presentHold` 4.2 s (was 0.7 / 2.4) — the still, flat reveal
  wants a long, calm beat.

**`joining` — flutter up and away, into the visible flock.** At the `present → joining`
handoff, `aimJoin(c, t)` shifts the butterfly's orbit `phase` (only — the orbit itself is
unchanged) so the point it reaches at the end of `joining` sits `CFG.revealJoinArc`
(1.05 rad) to one side of the visitor's gaze, on the side it already travels. The visitor
watches it join the others rather than peel away behind them; from there it drifts freely
(and eventually behind), which is the kaleidoscope. Then `tickJoining`:

- adds a half-sine **climb** (`CFG.revealJoinLift` 0.35 m) that peaks mid-flight and is
  back to zero when it meets the orbit — a straight `lerpVectors` slide becomes a takeoff;
- ramps the wings from the breath up to the **full flight beat** across the departure;
- eases the **group yaw** up from 0 to the travel heading (`c._joinYaw`, τ ≈ 0.38 s) and
  **slerps the model orientation** from the flat present pose (the group-identity look-at
  quat, re-expressed under the now-yawed group) to the flight's `presentRoll` aspect, with
  a **bank** (`CFG.revealJoinBank` 0.22 rad, half-sine, signed by orbit direction) through
  the turn. Mid-turn the wing does pass through edge-on briefly (~0.3 s) — a butterfly
  banking away shows you its edge, and the ambient flight's own aspect dips as low anyway;
- at `u = 1` writes the exact flight representation (`group.rotation.y = _joinYaw`,
  `model.rotation.set(rhoT, 0, 0)`) so `tickOrbit` continues it with no pop — verified:
  position, scale, yaw and roll are all continuous across the handoff.

`CFG.acceptResetDelay` is 5600 ms (was 3600) — still `≈ (presentArrive + presentHold)·1000`
plus a small margin, so the keyboard resets ~0.4 s into the departure.

All the reveal numbers are on-headset judgement calls — `config.js` has them with one-line
notes. Measured beat lengths: `present` ≈ 5.2 s, `joining` ≈ 3.2 s, orbit by ≈ 8.4 s.
...
- **"Here's your butterfly."** A freshly committed butterfly runs a three-state beat
  ...
  `separate()`. `CFG.acceptResetDelay` (4300 ms) resets the keyboard just as the butterfly
  starts leaving. **v7.2 art-directs `present` and `joining`** — see "The reveal (v7.2)"
  above; the shape of the state machine is unchanged.
