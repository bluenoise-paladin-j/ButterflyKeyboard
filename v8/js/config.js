...
var CFG = {
  ...
  acceptResetDelay: 5600,   // ms (v7.2: was 3600 -- kept ~= (presentArrive + presentHold)*1000 + 400, so the keyboard still resets just as the butterfly starts flying off)
  ...
  presentArrive: 1.0,       // seconds to rise into that spot (v7.2: was 0.7 -- the wings ease open into face-on rather than snap)
  presentHold:   4.2,       // seconds it hovers there, facing you (v7.2: was 2.4 -- the still, flat reveal wants a long, calm beat)
  presentJoin:   3.2,       // seconds to fly from there out to its orbit (v7.2: was 2.8 -- room for the flutter-up arc)

  //  v7.2 -- the reveal, art-directed. The presented butterfly hovers
  //  FLAT: wings square to the visitor like a pinned specimen, with only a
  //  slow shallow wing-breath. The flight's presentRoll CANNOT reach this
  //  aspect (it only ever presents a 3/4 view -- the body axis points at
  //  the visitor there), so tickPresent orients the model with a direct
  //  look-at (head up, wing plane normal -> the camera) instead. Then it
  //  flutters UP and away into the part of the kaleidoscope the visitor is
  //  looking at, wings rising to a full beat as it banks into the turn,
  //  the orientation slerping back to the flight's presentRoll on the way.
  //  See tickPresent / aimJoin / tickJoining in collection.js.
  revealFlapAmp:  0.11,     // dihedral of the subtle breath, radians -- bm.flap's pivots mirror, so the wing tips rise/fall together (the flight beats at ~1.0)
  revealFlapRate: 1.9,      // radians/sec of that breath -- slow and gentle (the flight's flapSpeed is 18-27)
  revealJoinLift: 0.35,     // metres the departure arcs upward at its midpoint, back to zero by the time it meets the orbit
  ...
  //  (see CLAUDE.md), for the hero butterfly. Two divergences from the
  //  Python: it is seeded off the NAME (a hash of entry.values) not an
  //  atlas index, drawn from mulberry32 not numpy's PCG64 -- the draw
  //  ORDER is reproduced exactly, the numbers are not; and its base hue is
  //  name-derived across the whole wheel, replacing the Python's fixed
  //  0.08 (orange) centre. Same name -> same texture on any machine.
  wingColRandAmt:  0.5,     // the Python's "Randamt": scales every jitter draw. 0 = the pure formula, only the name hue varies
  wingColPeriod:   1.0,     // the Python's "Period" (minimum): ramp-fold repeat. 1.0 = one sweep across the wing; higher = mirrored bands
  wingColRampType: 'random', // 'random' (per-name pick) | 'horiz' | 'vert' | 'radial' | 'circular'. The pick is ALWAYS drawn, even when overridden, for determinism
