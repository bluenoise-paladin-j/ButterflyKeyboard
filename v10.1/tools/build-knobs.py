...
#  constant that predates this version, 'new' for one v10 introduced,
...
FILES = [
 dict(f='js/config.js', hue=214, blurb='Every number the piece is built from. 213 keys. Almost everything you will want to move is here.', groups=[
  ('the room', [
  ...
  ('the session — v10’s guide, and the voice that runs it', [
   ('guideQuietMs','30000',884,"Milliseconds of no interaction before the nudge. SHORT on purpose: an exhibition floor hands the headset over fast, and ending is non-destructive — the collection is never touched, so a visitor ended early just presses start again.",'new'),
   ('guideQuietGrace','15000',885,"...and this much more before the farewell. ANY activity cancels and puts the session back, so nobody is cut off mid-action.",'new'),
   ('guideMaxMs','480000',886,"Hard cap on one session. An exhibition backstop — it is what ends a session the quiet clock can never end, because something is permanently freezing it.",'new'),
   ('guideDoffMs','2000',887,"Milliseconds of XR visibility ‘hidden’ that count as the headset coming off. Ends the session silently — there is nobody in there to hear a farewell.",'new'),
   ('guideUnlockAt','0',893,"Seconds into the welcome track before the room comes live. 0 = at once, so a visitor who already knows what to do is never blocked. Move it onto the second where the instruction lands once the track is cut.",'new'),
   ('guideWelcomeMax','60',894,"Seconds. A stalled welcome track can never hold the room locked longer than this.",'new'),
   ('guideFarewellMin','1.2',895,"Seconds the farewell beat lasts even with no track behind it — long enough for the flower to fade back in.",'new'),
   ('guideFarewellMax','20',896,"...and it can never last longer than this.",'new'),
   ('guideTalkMax','90',902,"The longest a cue may freeze the quiet clock. A track that stalls mid-fetch never fires ‘ended’ and never reports itself paused; without this, one bad file holds a session open until guideMaxMs.",'new'),
   ('guideCtlFade','0.50',904,"Seconds: accept and delete fading out between sessions. Linear travel read through a smoothstep, exactly as reveal.js does its envelope — so it lands on 0 and 1.",'new'),
   ('guideFlowerFade','0.70',905,"Seconds: the start flower fading in and out.",'new'),
   ('guideBlobX','0.00',911,"The start flower, on the panel — CFG.panelPos(x, y). Centred.",'new'),
   ('guideBlobY','-0.30',912,"Between the name field (−0.235) and the two controls (−0.435).",'new'),
   ('guideBlobK','1.60',913,"Multiplies blobW/blobH. Accept is 1.26 and delete 1.02, so the start flower reads as clearly the bigger thing — and in idle it is the only pickable thing in the room.",'new'),
   ('guideBlobSeed','1.80',914,"The lobe phase. Deliberately neither control’s (0.9 / 2.7), so the three never look like the same flower.",'new'),
   ('guideBlobHue','212',915,"Neither accept’s 142 nor delete’s 356. Saturation is 100, like every control.",'new'),
   ('guideBlobLit','44',916,"Per cent, at rest.",'new'),
   ('guideBlobLitHot','34',917,"...and pointed at. The same narrow lightness band the two controls carry state in.",'new'),
   ('guideBlobTilt','-0.18',918,"Its tilt in the plane.",'new'),
   ('guideBlobCant','0.10',919,"...and its cant out of it.",'new'),
   ('voCues','{ welcome, spell, pinch, recall }',929,"Cue name → file, relative to index.html. A cue with no entry is a SILENT NO-OP that still reports itself finished, so the guide runs with whatever subset of the tracks exists — nudge and farewell are not recorded. See audio/README.md.",'new'),
   ('voIntro','[welcome, spell, pinch]',960,"The opening, in order — three recordings played back to back as ONE cue. playing() stays true across the gaps, so the silence between two of them is never mistaken for the narration having finished.",'new'),
   ('voGap','0.45',961,"Seconds of silence between two cues in a sequence.",'new'),
  ]),
  ('palette', [
 ...
 dict(f='js/guide.js', hue=95, blurb='The session: idle → welcome → live → wrapup → farewell → idle. Every number it runs on is a CFG.guide* key above; these two are structural.', groups=[
  ('structure', [
   ('TALKS_OVER','{ nudge, farewell }',86,"The cues that do NOT freeze the quiet clock. The nudge is asking whether anyone is still there, so its own playback must not answer the question.",'new'),
   ('Escape','—',390,"The operator override: ends the session and hands over at once, from any state. Wired in the component’s init.",'new'),
  ]),
 ]),
 ...
 dict(f='js/voice.js', hue=340, blurb='The narration. One HTMLAudioElement per named cue, .volume for level, no AudioContext. A missing cue is a silent no-op that still finishes, so nothing ever waits on audio.', groups=[
  ('structure', [
   ('HAVE_AUDIO','typeof Audio !== ‘undefined’',51,"No Audio constructor → the whole module is a no-op. That is what lets tools/reach drive the guide headless.",'new'),
   ('unlock()','—',110,"Play-then-pause on every element, spent from ONE real DOM gesture. A hand-tracked pinch is not a gesture; the Enter-AR click is, and it always happens first.",'new'),
  ]),
 ]),
...
w('<div class="wrap">\n')
w('<header class="top">\n')
w('<p class="eyebrow">Butterflies VR · v10 · versions/v10</p>\n')
...
w('<span><b>%d</b> new or changed in v10</span>' % newn)
w('</div>\n</header>\n')
...
for f in FILES:
    ...
    for gname, rows in f['groups']:
        ...
        for name, val, line, note, flag in rows:
            ...
            elif flag=='changed': fl = '<span class="flag chg">v10</span>'
...
w('<span><code>versions/v10/</code></span>')
