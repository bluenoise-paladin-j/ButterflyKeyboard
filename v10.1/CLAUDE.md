...
machine has no node — against a maths-only THREE stub. **177 assertions** over four suites
...
**Ending is non-destructive**, and that is the whole argument for a 30-second default on a
floor with rapid handovers. It resets an empty keyboard, sends anything with the visitor home
...
making 30 s safe rather than a hair trigger: the reveal is ~14 s of watching by design,
...
| **Escape** | the operator override — ends the session and hands over at once, from any state |
...
file to change. `tools/knobs.html` is the searchable index of all **277** of them; it is
