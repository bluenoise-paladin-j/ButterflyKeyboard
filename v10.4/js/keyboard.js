...
AFRAME.registerComponent('butterfly-keyboard', {
  init: function () {
    ...
    this._nameExiting = false;         // v8.2: true while the caught name is bursting away (drives reset, sim-clock)
    this.keys = [];
    this.buttons = [];
    this.nameSprites = [];
    ...
    //  v8.1: the reveal's envelope and its one-shot shove, latched per frame
    this._dim = 1; this._deco = 1; this._shock = null; this._shockT = 0;
    ...
    //  v8.1 -- THE NAME HANDOVER. collection.js fires this on the frame
    //  the grown butterfly takes off. At that instant the visitor's name
    //  is in two places: hanging in front of them where they spelled it,
    //  and lit under the butterfly. The one in front flies apart upward
    //  and fades; the one on the butterfly stays and leaves with it.
    //
    //  It also replaces something worse. Until now the name simply
    //  VANISHED when acceptResetDelay fired and syncName() disposed the
    //  sprites mid-air.
    //
    //  v8.2: the reset used to be a wall-clock `setTimeout` here, timed to
    //  the fly-out's *sim-clock* duration. On any frame drop -- which
    //  happens -- the timer fired while the letters were still mid-air and
    //  syncName() disposed them, so the name "sometimes" vanished early.
    //  Now the fly-out completes itself (see tickUI) and calls reset() the
    //  frame the last letter is done; acceptResetDelay stays as the only
    //  wall-clock fallback, for when no butterfly ever launches.
    window.addEventListener('reveal:launch', function () {
      if (!self.done) { return; }             // only ever during the accept beat
      for (var i = 0; i < self.nameSprites.length; i++) {
        var sp = self.nameSprites[i];
        sp.flyFrom = null;                    // a late arrival is overtaken by the exit
        sp.flyOut = 0;
      }
      self._nameExiting = self.nameSprites.length > 0;
      if (!self._nameExiting) { self.reset(); }   // nothing to fly out
    });

    window.addEventListener('keydown', function (e) {
      // desktop convenience only; the piece itself never needs a keyboard
      if (self.done) { return; }
      //  v10: ...and it must not spell into a room that is not the
      //  visitor's yet. The pointer is locked out by the guide's
      //  exclusivity (interact.js:gather), but this handler bypasses
      //  interact.js entirely, so it needs the one gate of its own.
      if (typeof Guide !== 'undefined' && !Guide.roomLive()) { return; }
      //  v10.4: this handler goes straight to the keyboard, so it misses
      //  interact.js's click -- the one other place the select sound has
      //  to be said out loud.
      var took = true;
      ...
      if (took && typeof Sfx !== 'undefined') { Sfx.select(); }
    });
  },
  ...
  buildKeys: function () {
    for (var i = 0; i < CFG.letters.length; i++) {
      ...
      var k = {
        ...
        st: st, group: g, anchor: anchor, bm: bm, letter: lt, lead: lead,
        ...
        first: true, smoothRoll: 0, smoothPitch: 0,
      ...
      //  This key's slice of the arc, used only when CFG.arcSpan is not a
      //  full circle -- pathAt() sweeps theta between centre +- swing then.
      //  -PI/2 is straight ahead (x = r cos theta, z = r sin theta, the
      //  visitor faces -z). At the default full circle these go unread.
      var span = Math.min(CFG.arcSpan, Math.PI * 2);
      k.centre = -Math.PI / 2 + (i / (CFG.letters.length - 1) - 0.5) * span * 0.78;
      k.swing = span * 0.09;
  ...
  accept: function () {
    ...
    var self = this;
    //  v8.1: A FALLBACK, not the timing. The reset normally comes from
    //  'reveal:launch' (see init) once the name has flown, so this number
    //  no longer has to be hand-kept equal to a sum of the reveal's beat
    //  durations -- which went stale every time the reveal was retuned.
    //  This only fires if no butterfly ever arrives at all: a backgrounded
    //  tab whose render loop is throttled so collection.js never drains
    //  its queue, or a name the seam refuses. reset() is idempotent, so
    //  whichever path gets there first wins.
    setTimeout(function () { self.reset(); }, CFG.acceptResetDelay);
  },

  reset: function () {
    this.typed = '';
    this.done = false;
    this._nameExiting = false;
  ...
  tick: function (time, dtMs) {
    ...
    //  v8.1 -- the reveal's envelope, read once a frame. `dim` fades
    //  everything that is not the hero butterfly; `deco` is the same ramp
    //  taken all the way to zero, for the things that should go entirely
    //  rather than merely recede -- the letters' ghosts and lattices, and
    //  the two controls, which are unusable during the beat anyway
    //  (targets() returns nothing once `done`). Both are exactly 1 with
    //  no reveal running, so this is v8's keyboard untouched.
    this._dim = Reveal.dim;
    this._deco = (CFG.revealDim >= 1) ? 1
      : Math.max(0, Math.min(1, (this._dim - CFG.revealDim) / (1 - CFG.revealDim)));

    //  The hero's takeoff publishes one radial impulse; apply it on the
    //  frame it changes. See tickKey.
  ...
  updateSlowField: function (dt) {
    ...
    for (i = 0; i < this.keys.length; i++) {
      ...
      //  v8.1 -- while the reveal has the room, the whole swarm is
      //  ceilinged at Reveal.slow. Dimming alone leaves twenty-six
      //  butterflies still darting about behind the one thing the visitor
      //  is meant to be looking at; calming them is the stronger half of
      //  the isolation, and it costs one line because this field already
      //  exists, is already eased, and is already tested on-headset.
      //  Reveal.slow is exactly 1 with no reveal running.
      if (Reveal.slow < target) { target = Reveal.slow; }
  ...
  tickUI: function (t, dt) {
    ...
    for (i = 0; i < this.nameSprites.length; i++) {
      ...
      if (sp.flyOut != null) {
        //  v8.1/v8.2 -- THE HANDOVER. The butterfly has just launched
        //  UPWARD, and the name it leaves behind is blasted DOWNWARD --
        //  each letter thrown down on an accelerating fall (flyOut^2, like
        //  wash off the takeoff), fanning outward, spinning, and fading. It
        //  lingers ~nameExitTime so it is still falling while the butterfly
        //  soars, rather than snapping away and leaving a gap. The scatter
        //  is deterministic (off Style.forNameSlot, already on the sprite),
        //  so a name always comes apart the same way.
        sp.flyOut = Math.min(1, sp.flyOut + dt / CFG.nameExitTime);
        var fo = smoothstep(sp.flyOut);
        var fall = sp.flyOut * sp.flyOut;                 // accelerating -- blast wash, then gravity
        var dirX = (slotX >= 0 ? 1 : -1);                 // flung outward from the word's centre
        sp.position.set(slotX + (dirX * CFG.nameExitSpread + sp.st.overX * 1.5) * fo,
                        slotY - CFG.nameExitDrop * fall,
                        sp.st.overY * 0.6 * fo);
        //  fully faded by ~70% through, so the tail of the fall is unseen
        var fade = 1 - smoothstep(Math.min(1, sp.flyOut / 0.7));
        sp.set(this.nameScale * this.confirmScale * (1 + 0.35 * fo), fade * 0.95);
        if (sp.spin) { sp.spin(sp.st.rot + dirX * fall * 3.0); }
      } else if (sp.flyFrom) {
    ...
    //  v8.2: the exit drives its own reset -- the frame every letter has
    //  finished bursting away, sim-clock, so a frame drop can never dispose
    //  them mid-fall the way the old wall-clock setTimeout did.
    if (this._nameExiting && this.nameSprites.length) {
      var done = true;
      for (i = 0; i < this.nameSprites.length; i++) {
        if ((this.nameSprites[i].flyOut || 0) < 1) { done = false; break; }
      }
      if (done) { this.reset(); }
    }
    ...
    for (i = 0; i < this.buttons.length; i++) {
      ...
      //  v8.1: the two controls go entirely, not merely dim, while the
      //  reveal has the room. They are unusable then anyway -- targets()
      //  returns nothing once `done` -- so leaving them sitting there was
      //  only ever clutter in front of the one thing worth looking at.
      //  v10: ...and the same is true between sessions. In idle there is
      //  nothing to accept or delete and one flower to press, so the two
      //  of them hanging there dim is exactly the clutter this line
      //  already exists to remove. Same scalar path, one more factor.
      b.ui.setAlpha(this._deco * (typeof Guide !== 'undefined' ? Guide.ctlAlpha() : 1));
    }
  },
  ...
  tickKey: function (k, t, dt) {
    ...
    if (this._shock && CFG.revealShock) {
      this._tmp.copy(k.pos).sub(this._shock);
      var sd = Math.max(0.6, this._tmp.length());   // clamped, or one right at the launch point is flung
      if (this._tmp.lengthSq() > 1e-8) {
        k.offsetVel.addScaledVector(this._tmp.normalize(), CFG.revealShock / (1 + sd * sd));
      }
    }
    ...
    if (hSpeed > 1e-6) {
      ...
      k.smoothPitch += (pitch - k.smoothPitch) * Math.min(1, dt * 1000 / CFG.pitchEase);
      ...
      k.bm.model.rotation.set(this.presentRoll(k, mean) + k.smoothRoll, 0, k.smoothPitch, 'XZY');
    }
    k.prev.copy(k.pos);
  },
  ...
  dress: function (k, scaleMul, alpha) {
    ...
    //  v8.1: the reveal's dim. `visible` deliberately still tests the
    //  UNDIMMED alpha -- gating it on the dimmed one would pop the whole
    //  keyboard off and back on as the envelope crosses 0.01.
    var dim = this._dim;
    k.bm.model.scale.setScalar(k.size * k.scale);
    k.bm.setOpacity(k.alpha * dim);
    ...
    k.letter.setDeco(this._deco);           // v8.1: its ghosts and lattice go during a reveal
    var lp = k.letter.place(this._camPos, k.pos, k.size * k.scale, k.alpha * dim, this._clock);
    ...
    if (k.lead) {
      ...
      k.lead.material.opacity = 0.55 * k.alpha * dim;
    }
  },
