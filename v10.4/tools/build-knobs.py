...
#  constant that predates this version, 'new' for one v10.4 introduced,
...
FILES = [
 dict(f='js/config.js', hue=214, blurb='Every number the piece is built from. 228 keys. Almost everything you will want to move is here.', groups=[
  ...
  ('the session — v10’s guide, and the voice that runs it', [
   ('guideQuietMs','10000',917,"Milliseconds of no interaction before the nudge. v10.2: was 30000 — fifteen seconds from the last thing a visitor did to the room handing over, where v10 took forty-five. Ending is non-destructive, so the cost of ending early is that they press the flower again. This and the headset coming off are now the only two ends there are.",''),
   ('guideQuietGrace','5000',918,"...and this much more before the farewell. ANY activity cancels and puts the session back. v10.2: was 15000.",''),
   ...
   ('voIntroDelay','0.90',1101,"Seconds between the start flower being pressed and the first spoken word. v10.4: the press has its own sound now, and the two arriving on the same frame meant neither read. The welcome state HOLDS across this pause — it cannot fall through to live in the silence.",'new'),
  ]),
  ('the room\u2019s sound \u2014 js/sfx.js (v10.4)', [
   ('sfx','{ music, wing1, wing2, select }',1107,"Name \u2192 file, relative to index.html, exactly as voCues is. The four are in audio/sfx/; the masters are sounds/sfx/. Everything here is a level write on a tick \u2014 there is no AudioContext anywhere in the build.",'new'),
   ('sfxWings','[wing1, wing2]',1115,"Which entries above are wingbeats. Add a third by dropping the file in, naming it in sfx, and listing it here.",'new'),
   ('sfxMusicGain','0.50',1121,"THE knob for how loud the room is. Measured off the masters: the bed is \u221228.5 dB RMS and the wingbeats \u221242.8 / \u221246.8, i.e. 15\u201318 dB under it \u2014 so the wings run at full and the BED is the one pulled down. Raise or lower this, not sfxWingGain.",'new'),
   ('sfxMusicFadeIn','3.00',1126,"Seconds, up from silence at the head of the track. currentTime resets when the element loops, so this is BOTH the opening fade-in and half of every loop seam \u2014 there is no crossfade and no second element.",'new'),
   ('sfxMusicFadeOut','2.50',1127,"Seconds, down to silence off the end. The master starts at 0.116 RMS and ends at 0.003, so a bare loop bangs once every 2:21; this is what stops it, and it hides the AAC padding at the join for free.",'new'),
   ('sfxWingGain','1.00',1129,"Full, and it should stay there \u2014 see sfxMusicGain.",'new'),
   ('sfxWingFade','0.60',1130,"Seconds, up at the head and down at the tail of each wingbeat. Same envelope helper as the bed.",'new'),
   ('sfxWingGapMin','4.00',1131,"Seconds of quiet between two wingbeats\u2026",'new'),
   ('sfxWingGapMax','14.00',1132,"\u2026picked uniformly in this band. They are atmosphere: nothing triggers them and nothing waits on them.",'new'),
   ('sfxWingFirstGap','2.00',1133,"\u2026and before the first one.",'new'),
   ('sfxSelectGain','0.90',1135,"The click, on everything selected \u2014 the flower, accept, delete, a letter, a collected butterfly.",'new'),
   ('sfxSelectVoices','4',1139,"Elements in the click POOL. The recording runs 1.85 s and a visitor spelling a name presses faster than that; retriggering ONE element would cut every second press dead.",'new'),
   ('sfxDuck','0.35',1143,"What the bed and the wingbeats drop to while the narration speaks. The piece talking always wins. This is the seam Voice.duck() was left open for in v10, used from the other side.",'new'),
   ('sfxDuckFade','0.50',1144,"Seconds, eased both ways \u2014 so the return is not a step on the frame a cue ends.",'new'),
  ]),
  ('palette', [
...
w('<div class="wrap">\n')
w('<header class="top">\n')
w('<p class="eyebrow">Butterflies VR · v10.4 · versions/v10.4</p>\n')
...
w('<span><b>%d</b> new or changed in v10.4</span>' % newn)
w('</div>\n</header>\n')
...
for f in FILES:
    ...
    for gname, rows in f['groups']:
        ...
        for name, val, line, note, flag in rows:
            ...
            elif flag=='changed': fl = '<span class="flag chg">v10.4</span>'
...
w('<span><code>versions/v10.4/</code></span>')
