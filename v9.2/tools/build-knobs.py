...
FILES = [
 dict(f='js/config.js', hue=214, blurb='Every number the piece is built from. 173 keys. Almost everything you will want to move is here.', groups=[
  ('the room', [
  ...
  ('the swarm — the 26 keyboard butterflies', [
   ...
   ('hgtMax','1.95',40,"Highest flight height, metres. v9 CUT THIS from 2.30: the keyboard is now a LOW dome and the collection a high one above it, so looking up past the letters means looking at nothing but your own butterflies. The only number in the keyboard's own flight v9 moves.",'changed'),
  ...
  ('the collection — the kaleidoscope that accumulates', [
   ...
   ('colRadMin','1.7',337,"Nearest collection orbit. v9 brought the whole band IN as well as up — closer and higher work together: at 1.7 m out it takes only 2.2 m of height to clear the letters, where at 2.6 m it would have taken 2.9.",'changed'),
   ('colRadMax','3.0',337,"Furthest collection orbit. Overlaps the keyboard's 1.0–2.4 in plan on purpose: the two are separated in HEIGHT, so they sit at different elevations rather than merely different depths along one ray.",'changed'),
   ('colHgtMin','2.2',338,"Bottom of the collection's dome — above the letters entirely. It used to be 0.8, which put the whole kaleidoscope INSIDE the letters' angular span, so every collected butterfly had letters in front of it from somewhere and the pick could not get past them.",'changed'),
   ('colHgtMax','3.1',338,"Top of it. Kept under 3.1 deliberately: in AR passthrough anything higher goes through a real ceiling.",'changed'),
  ...
  ('calling a collected butterfly over (v9) — selecting one', [
   ('colPickRadius','0.20',575,"Pick sphere, multiplying the butterfly's own size. The keys' own rule, so a collected one is no easier or harder to hit than a letter.",'new'),
   ('colHiScale','1.20',576,"How much a highlighted one grows. Lower than the keys' hiScale 1.45 — these are bigger and further away.",'new'),
   ('summonMax','1',577,"How many can be called over at once. Calling another sends the longest-held one home rather than refusing.",'new'),
  ...
  ('…it flies over', [
   ('summonSpeed','0.75',595,"Metres/sec cruising in. A cruise, not a lerp: a lerp is fastest when furthest away, which is backwards for something flying to your hand.",'new'),
   ('summonSpeedNear','0.22',596,"Metres/sec at the very end, so it arrives settled rather than stopping dead.",'new'),
   ('summonEase','0.60',597,"Metres over which it eases from one to the other. At the first pass (1.00 m) the crawl over the last metre dominated the trip.",'new'),
   ('summonSway','0.09',598,"Metres of side-to-side drift on the way in — what stops the approach reading as a dolly move.",'new'),
   ('summonBob','0.05',599,"Metres of rise and fall with it.",'new'),
   ('summonSwayRate','1.5',600,"Radians/sec of both. Added as a VELOCITY, so the fade-out near the target can never step.",'new'),
   ('summonArrive','0.085',601,"Metres from the target that counts as arrived.",'new'),
   ('summonGiveUp','30',602,"Seconds before it gives up and goes home — a safety net for a palm it can never quite reach.",'new'),
  ]),
  ('…no hand: it hovers in front of you', [
   ...
   ('hoverSpanX','0.14',690,"Metres it wanders side to side. IT FLIES while it waits — the first pass held it in the reveal's flat pinned-specimen pose and it read as a diagram of a butterfly rather than a butterfly.",'new'),
   ('hoverSpanY','0.09',691,"Metres up and down.",'new'),
   ('hoverSpanZ','0.06',692,"Metres toward and away — least, or it keeps changing size.",'new'),
   ('hoverRate','9',693,"Multiplies the butterfly's OWN orbit noise frequencies, so no two wait the same way. Cut from 20 on the second pass — a full wander now takes 3–6 s. Measured over the whole 7 s wait: 0.09 m of drift at a 0.19 m/s peak, against 0.24 m at 0.36 before.",'new'),
   ('hoverEase','0.40',694,"Seconds of lag onto the wandering point. THIS is the damping — raise it to calm the movement without making it smaller. The wander is applied to the target and the position lags it; steering the position directly leaves everything chasing noise.",'new'),
   ('hoverSwayYaw','0.30',708,"Radians the held heading sways either side, about 17°. THE HEADING IS HELD, NOT CHASED: following its own travel direction swung the body a full half turn every time the wander reversed — 4.6 rad of yaw in 3.3 s, which is what read as frantic. It now holds BROADSIDE to the visitor (facing them square is the one heading at which the wing plane can never face them either) and sways about it: 0.9 rad over the same window.",'new'),
   ('hoverTurn','0.9',709,"Seconds the yaw takes to ease onto that heading. Long, so the sway reads as a drift rather than a turn. Also covers the arrival, where it turns from the heading it flew in on to the nearer broadside — at most a quarter turn, because the side is chosen as the shorter one.",'new'),
   ('hoverSettle','0.7',710,"Seconds the wander ramps in over as it arrives. Without it the target jumps on the first frame of the hover and it lunges at the visitor's face at 1.0 m/s; ramped, the peak over the whole wait is 0.36.",'new'),
  ]),
  ('…a hand: it lands on it', [
   ('perchYawMin','60',714,"DEGREES (this file is otherwise radians; these two are not). How far across the visitor's view it sits when perched. The body is ONE PLANE through the body axis, so head-on the visitor looks straight down its length and it disappears — two wings with nothing joining them. Drawn fresh each landing between the two, and the side is drawn too. Set both to 0 for the head-on pose.",'new'),
   ('perchYawMax','70',715,"The other end of that draw. Set both the same to pin it.",'new'),
   ('perchSize','0.50',734,"About 18 cm across the wings against a ~9 cm palm. THE number to move if the perch looks wrong in the headset.",'new'),
   ('perchPitch','0.205',789,"IT STANDS NOSE-UP, because the legs are not all the same length (v9.2). BODY_ALPHA draws a body in FLIGHT seen side-on — front legs long and forward, hind pair short and swept back — so laid flat on the deepest leg the hind tips floated 22 and 25 mm over the palm and the tail end of the butterfly did not rest on the hand at all. Radians of nose-up tilt, about the wing SPAN axis so the wings stay level across the palm and only tip fore-and-aft. 0.205 (11.8°) is the least-squares line through the four measured leg tips: hind pair down within a millimetre, mid pair 8.6 mm into the hand. 0.323 is the lower hull — nothing sinks, three of four land exactly, but 18.5° reads as rearing rather than settling. Set 0 for v9.1's flat pose.",'new'),
   ('perchLegDrop','0.1145',790,"IT STANDS ON ITS LEGS. Per unit model size, how far the legs' CONTACT LINE sits below the model's origin — and the origin is the WING HINGE, so placing that on the palm (which the first pass did) buried the body and every leg in the hand, about 52 mm of it. Measured off BODY_ALPHA itself: decode it, threshold at the material's own alphaTest of 0.5, label the four leg strands, and fit the line perchPitch pitches onto; the drop is measured perpendicular to it. Was 0.1406, the single lowest painted pixel, while the pose was flat. Cross-check: tagBodyDrop, measured independently for the name tag, is −0.135.",'changed'),
   ('perchLift','0.002',791,"Metres of clearance between that contact line and the palm. Negative sinks it in.",'new'),
   ('perchFollow','0.05',637,"Seconds of lag following the hand. A butterfly welded to a jittering palm reads as a decal.",'new'),
   ('perchDwell','12.0',638,"Seconds it stays, if the hand stays.",'new'),
   ('perchSettle','0.30',639,"Seconds of folding its wings down out of the approach as it touches.",'new'),
   ('perchRest','-0.30',640,"Radians of dihedral at rest.",'new'),
   ('perchFlutterEvery','2.4',646,"Seconds between flutters, jittered ×0.6–1.5 so two butterflies never sync up.",'new'),
   ('perchFlutterFor','0.55',647,"Seconds each flutter lasts — three strokes under one envelope.",'new'),
   ('perchFlutterAmp','0.75',648,"Radians at the top of one.",'new'),
  ]),
  ('…and it goes back', [
   ('leaveTime','3.0',656,"Seconds from wherever it is back onto its orbit.",'new'),
   ('leaveLift','0.30',657,"Metres it arcs upward at the midpoint of that.",'new'),
   ('leaveBank','0.22',658,"Radians of bank through the turn away, peaking mid-departure.",'new'),
   ('leaveArc','0.55',659,"Radians round its own ring ahead of the point it left from — turns a radial exit into a curve.",'new'),
  ]),
  ('the posture that means “land here” (hands.js:palmPose)', [
   ('palmUpDot','0.62',682,"dot(palm normal, world up) — about 52° off vertical. Generous on purpose: the failure mode of a tight threshold is an interaction that mysteriously does not work.",'new'),
   ('palmFlatExtend','1.45',683,"Mean fingertip-to-wrist distance, in PALM WIDTHS. ~1.9 flat, ~1.0 in a fist — a wide gap, and the test doing the work.",'new'),
   ('palmFlatOffset','0.62',684,"Mean fingertip distance off the palm plane, in palm widths. ~0.2 flat, 0.5–0.75 curled. A lenient backstop against a cupped hand.",'new'),
   ('palmRaiseBelowEye','0.50',685,"Metres under the HEADSET the palm may be and still count as raised — not metres off the floor, so it works seated and at any height.",'new'),
   ('palmHoldMs','220',686,"Ms the whole pose must hold before it counts as an offer.",'new'),
   ('palmGraceMs','320',687,"Ms it must be gone before the offer is withdrawn. A tracking dropout is not a withdrawn hand.",'new'),
   ('palmSimOut','0.42',694,"THE DESKTOP STAND-IN: metres in front of the camera SPACE puts a synthetic palm. Refused inside an XR session.",'new'),
   ('palmSimDrop','0.40',695,"…and metres below it.",'new'),
  ]),
