...
var CFG = {
  ...
  //  v8.3 opened the generator up (see "The base-colour generator"):
  //  eight ramp kinds, a finer fold carved into the ramp, and two-tone
  //  palettes that sweep between a name's hue and a harmonic second hue.
  wingColRandAmt:  0.62,    // "Randamt": scales EVERY jitter draw (palette spread, sat/val, warp, ramp centre...). 0 = pure formula, only the name hue + harmony vary. v8.3: 0.5 -> 0.62
  wingColPeriod:   1.0,     // band-fold repeat MINIMUM. 1.0 = one sweep across the wing; the draw skews up toward wingColPeriodMax, so bands are the norm
  wingColPeriodMax: 7.0,    // v8.3: band-fold repeat CEILING. The per-name period is drawn in [wingColPeriod, this]. Raise for finer/tighter bands
  wingColRampType: 'random', // 'random' (per-name pick) | horiz | vert | diag | diagalt | radial | circular | spiral | plaid. The pick is ALWAYS drawn, even when overridden, for determinism
  wingColHarmony:  'random', // v8.3: the second anchor hue's relation to the name hue. 'random' (per-name pick) | 'complement' | 'triad' | 'split' | 'analog'
