# v8.3 — the wing colour, layered, 7 September 2026
...
v8 gave the grown butterfly a per-name procedural wing colour — a faithful-look
port of `wingbasecolour_script.py`. Faithful, and tame: one global gradient ramp
that read as wallpaper. **v8.3 rebuilds it as a layered generator** on feedback —
"more butterfly-esque… incorporate more layers": a membrane wash with venation,
eyespots, a marginal band + submarginal spot row, a discal mark, scale speckle
and basal suffusion composited on top, all seeded from the name. Two-tone
harmonic palettes. Only `js/wing-colour.js`, `js/config.js` and
`tools/colour-preview.html`; no new files; the reveal, the flight, the DNA path,
wing parity and persistence are all untouched.

## What changed from v8.2

`wing-colour.js` is no longer a line-by-line port — it is `wing-colour.js`'s own
**layered** generator, still deterministic from the name, still a faithful
*look*. v8's one global gradient ramp read as wallpaper; v8.3 builds the wing the
way a real one is built — a membrane wash with discrete markings composited on
top. In order:

1. **Membrane** — the old ramp, now with **nine kinds** (`margin` — bands
   parallel to the wing edge, weighted heavily — plus `horiz` `vert` `diag`
   `diagalt` `radial` `circular` `spiral` `plaid`), a fine fold carved in for
   striation, and an fBm warp. Coloured through a **two-tone palette**: each
   name derives a second anchor hue from its own by a harmonic rule
   (complement / triad / split / analogous — per name or `CFG.wingColHarmony`),
   and the palette sweeps between the two on the **short** way round the wheel.
   The field is biased toward the name's own hue so the second reads as accent.
2. **Wing division** — fore (`v<0.5`) and hind take a slight value offset.
3. **Scale speckle** — a crisp two-octave per-pixel grain, for the powdery read.
4. **Mottle** — the second palette-coloured fBm, as v8.
5. **Venation** — 3–5 tapering veins per wing fanning from a convergence node
   *off* the root edge (hidden under the body on a real wing) out to the margin,
   subtly darkening, optionally cross-linked. The strongest "this is a wing" cue
   after the eyespots.
6. **Discal mark** — an optional dark spot mid-wing.
7. **Eyespots** — 0–4 ocelli (dark pupil, bright rim, coloured iris, dark ring,
   pale halo, white catch-light). When there are several they form a
   **submarginal row** along the outer wing, not a scatter.
8. **Marginal band + submarginal lunules** — a solid dark border, with a row of
   soft pale/coloured spots just inside it along the outer edge only.
9. **Basal suffusion** — dark against the body at the root.
10. **Overlay tint** + a final saturate/deepen grade.

**Two genuinely new knobs**: `CFG.wingColPeriodMax` (7 — band-fold ceiling) and
`CFG.wingColHarmony` (`'random'` | `complement` | `triad` | `split` | `analog`).
`wingColRandAmt` 0.5 → **0.62**. `colour-preview.html` has a control for all five
and still runs the determinism check.

**Every already-collected butterfly re-renders on next load** — the draw order
changed. Wing *shape*, the name hue the body and letters use, and the name tag
are all unaffected; only the wing colour/pattern is richer. This was the
explicit call.

**Cost**: ~20–30 ms per texture (was ~3 ms), once per name on a free frame like
the wing shape, then LRU-cached. The venation distance field is the bulk of it.

Everything below is the v8.2 note, kept for context.
