# Butterfly Keyboard — v8.3

**v8.3 rebuilds the wing colour as a layered generator.** v8's base-colour generator was
one global gradient ramp and read as "too basic and simple… not butterfly-esque". v8.3
makes `wing-colour.js` its own generator (still deterministic from the name, still a
faithful *look*): a **membrane wash** — nine ramp kinds incl. `margin` (bands parallel to
the wing edge), a fine fold, an fBm warp, through a **two-tone harmonic palette** — with
discrete markings composited over it: **venation**, **eyespots** (which form a submarginal
row when there are several), a **marginal band + submarginal spot row**, a **discal
mark**, **scale speckle** and **basal suffusion**. Two new knobs —
`CFG.wingColPeriodMax`, `CFG.wingColHarmony` — plus `wingColRandAmt` 0.5 → 0.62. Only
`js/wing-colour.js`, `js/config.js`, `tools/colour-preview.html`; no new files. **Every
collected butterfly re-renders its wing colour on next load** (the draw order changed);
shape, name hue and name tag are unaffected. See "The base-colour generator" below and
`VERSION.md`.
...
js/
  ...
  wing-colour.js    v8 / v8.3 — a name -> a base-colour texture (collection only; not parity-locked)
...
## The base-colour generator (v8, opened up in v8.3)

The wing **shape** is `wing-gen.js`, a parity-locked port of `wingtexgen_script.py`. v8
ported the sibling script, `wingbasecolour_script.py` (the TouchDesigner "BASE COLOR
GENERATOR v3"), into `wing-colour.js` — the wing **colour**. **v8.3 replaced that port
with `wing-colour.js`'s own generator**: the same lineage and the same faithful *look*,
but no longer line-for-line, because the port read as too plain. The generation stage is
whole: a name grows both shape and colour, deterministically.
...
the wing material *both* (`map` = colour, `alphaMap` = shape). The generator is
**layered** — all PRNG draws up front (the determinism contract), then a pixel loop that
composites, in order:

1. **Membrane** — a mirror-fold triangle-wave ramp (`foldTri`) in one of nine kinds:
   `margin` (bands parallel to the wing edge, corners rounded — weighted heavily), plus
   `horiz` `vert` `diag` `diagalt` `radial` `circular` `spiral` `plaid`. `'random'` picks
   from a **weighted bag** (`RAMP_BAG`) favouring `margin`/`circular`; `CFG.wingColRampType`
   forces one. A **finer fold** (2–5× the period, `fineAmt`) is carved in for striation;
   an **fBm warp** perturbs it. `CFG.wingColPeriod` is the band minimum,
   `CFG.wingColPeriodMax` (7) the ceiling. Coloured through a **two-tone HSV palette**:
   2–5 stops sweeping the name's hue → a **harmonic second hue** (`hueB`: complement /
   triad / split / analogous, per name or `CFG.wingColHarmony`) on the **short** way round
   the wheel, dark → bright. The field is biased toward the name's hue (`t0²(2−t0)`) so
   the second reads as accent and the wing still matches the body and letters.
2. **Wing division** — fore (`v<0.5`) / hind take a slight value offset.
3. **Scale speckle** — a crisp two-octave per-pixel hash grain (`speckAmt`).
4. **Mottle** — a second palette-coloured fBm, anchors reversed (`namt`).
5. **Venation** — 3–5 tapering veins per wing (`buildVeins`), fanning from a convergence
   node held *off* the root edge so only the diverged run shows; a per-pixel min-distance
   field (`veinDistSq`, AABB-culled) darkens near them; they thin and fade toward the tip
   and stop at the marginal band. Optional cross-veins.
6. **Discal mark** — an optional dark spot mid-wing (`hasDiscal`).
7. **Eyespots** — 0–4 ocelli: dark pupil, bright rim, coloured iris (`hueB`), dark ring,
   pale halo, white catch-light. Several ⇒ a **submarginal row** along the outer wing.
8. **Marginal band + submarginal lunules** — a solid dark border (`marginW`) with a row
   of soft pale/coloured spots (`lunCol`, `lunN`, `lunDepth`) just inside it, **outer
   edge only** (a row wrapping all four sides reads as a picture frame).
9. **Basal suffusion** — dark against the body at the root (`basalAmt`).
10. **Overlay tint** (`oMult` biased toward multiply) + a final saturate/deepen grade.

**Cost** ~20–30 ms/texture (v8 was ~3 ms) — once per name on a free frame, then
LRU-cached. The venation distance field is most of it.

**Not parity-locked.** The Python seeds numpy's PCG64 off an integer *atlas index*; here
the texture is the visitor's, so it is seeded off an FNV-1a hash of `entry.values` (same
construction as `Wings.hashValues`) and drawn from **`mulberry32`** (`web/js/prng.js`).
There is no TouchDesigner colour reference or harness in the repo and the texture is
name-driven, so a bit-match was never the contract. The contract is: **same
`entry.values` → byte-identical texture, on any machine, across reloads** — held by every
draw being deterministic from the seed in the fixed order the `DRAW N` comments mark.
v8.3 deliberately reordered/added draws, so v8 textures do not carry forward — see
`VERSION.md`. Base hue is **name-derived across the whole wheel** (`nameHueFor` — the
values reversed then hashed, so it is decorrelated from the draw seed), not the Python's
fixed `0.08` orange.
...
| key | default | |
|---|---|---|
| `wingColRandAmt` | 0.62 | scales **every** jitter draw (palette spread, sat/val, warp, marking sizes…). 0 = the pure formula, only the name hue + harmony vary. Lower it to calm the whole set down |
| `wingColPeriod` | 1.0 | band-fold repeat **minimum** |
| `wingColPeriodMax` | 7.0 | band-fold repeat **ceiling** (v8.3). Per-name period is drawn in `[min, max]`. Raise for finer/tighter bands |
| `wingColRampType` | `'random'` | `'random'` (weighted per-name pick) or a forced kind: `margin` `horiz` `vert` `diag` `diagalt` `radial` `circular` `spiral` `plaid`. `margin` / `circular` are the most wing-like. The pick is *always drawn* even when forced, for determinism |
| `wingColHarmony` | `'random'` | the second anchor hue's relation to the name hue (v8.3): `'random'` or `complement` / `triad` / `split` / `analog` |

`tools/colour-preview.html` has a control for each of the five and still runs the
"same name twice → byte-identical" determinism check. A dev aid, not needed to run.
