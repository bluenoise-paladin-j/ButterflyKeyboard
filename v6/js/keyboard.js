...
AFRAME.registerComponent('butterfly-keyboard', {
  ...
  tickKey: function (k, t, dt) {
    ...
    // ---- v2's flight, unchanged ----
    this.pathAt(k, t, k.pathPos);
    ...
    k.flapEnv += ((k.gliding ? 0 : 1) - k.flapEnv) * Math.min(1, dt / 0.22);

    var fp = t * k.flapSpeed + k.flapPh;
    var flapAngle = Math.sin(fp) * k.flapAmp - 0.5;              // beating, biased upward
    var glideAngle = -1.0 + 0.08 * Math.sin(t * 3 + k.flapPh);   // held up, tiny tremble
